  ...
  <div class="dark-l1 login page">
    ...
    <v-layout column
      ...
      <v-flex xs12 style="margin-top: 16vh;">
        <transition name="auth-animation" mode="out-in" enter-active-class="animated faster fadeInDown"
                    leave-active-class="animated faster fadeOutUp">
          <v-form v-if="loggingIn" ref="form" v-model="valid" class="animated faster fadeInDown">
            <v-text-field dark
                          box
                          name="username"
                          color="info darken-1"
            ...
            <v-layout row wrap fill-height justify-start align-center>
              ...
              <v-flex xs4>
                <v-btn @click="toggleRegistration()" :color="`success darken-1`" name="register">
                  <v-icon dark class="pr-2">how_to_reg</v-icon>
                  Sign Up
                </v-btn>
              </v-flex>
              <v-flex xs12>
                <v-btn flat name="Forgot Password" @click="resetDialog = true" :color="`info darken-1`">
        ...
        <v-dialog v-model="resetDialog" width="500">
          <v-card tile flat color="accent">
            <v-card-title primary-title class="hildaLight space-small ma-0 darkCard">Password Reset
              <v-spacer></v-spacer>
              <v-card-actions>
                <v-spacer></v-spacer>
                <v-btn color="primary"
