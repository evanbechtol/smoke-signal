    ...
    <v-dialog v-model="susDialog" :fullscreen="breakpoint === 'xs'">
      <v-card tile flat>
        <v-card-title primary-title class="hildaLight space-small">
          ...
          <v-spacer></v-spacer>
          <v-btn color="primary" flat @click="toggleSusDialog">
        ...
        <v-card-text>
          <v-layout row wrap align-start justify-start>
            <v-flex xs12>
              ...
              <p>
                The System Usability Scale (SUS) provides a “quick and dirty”,
                reliable tool for measuring usability. It consists of a 10 item
                questionnaire with five response options for respondents; from
                Strongly agree to Strongly disagree. Originally created by John
                Brooke in 1986, it allows you to evaluate a wide variety of
            ...
            <v-flex xs12 pb-3>
              ...
              <div>
                <ul>
                  <li>
                    Interpreting scoring can be complex. The participant’s
                    scores for each question are converted to a new number,
                    added together and then multiplied by 2.5 to convert the
                    original scores of 0-40 to 0-100.
                    <b
                      >Though the scores are 0-100, these are not percentages
                      and should be considered only in terms of their percentile
                      ranking.</b
                    >
                  </li>
                  <li>
                    Based on research, a SUS score above a 68 would be
                    considered above average and anything below 68 is below
                    average, however the best way to interpret your results
                    involves “normalizing” the scores to produce a percentile
                    ranking.
                  </li>
                </ul>
              </div>
            ...
            <v-flex xs12>
              ...
              <div>
                <ul>
                  ...
                  <li>
                    There is a temptation, when you look at the scores, since
                    they are on a scale of 0-100, to interpret them as
                  ...
                  <li>
                    SUS is not diagnostic - its use is in classifying the ease
                    of use of the site, application or environment being tested
                  </li>
                </ul>
              </div>
            </v-flex>
      ...
      <v-flex xs12>
        ...
        >
          ...
          >
            <v-card-title
              primary-title
              class="space-small hildaLight ml-2 pa-2"
            >
            ...
            <v-card-text
              :class="$vuetify.breakpoint.smAndUp ? 'mx-auto' : 'mx-0 px-0'"
            >
              <v-layout
                :class="$vuetify.breakpoint.smAndUp ? 'row' : 'column'"
                wrap
              >
                <v-flex
                  shrink
                  ...
                  <div>
                    <v-tooltip right class="menuIcon">
                      <v-btn
                        ...
                        slot="activator"
                        ><v-icon color="#b1b3b4">help</v-icon></v-btn
                      >
        ...
        >
          ...
          >
            ...
            <v-card-text v-if="avgRating !== null">
              <span class="space-small">Average Rating:</span
              ><v-rating
                v-model="avgRating"
                half-increments
                readonly
            ...
            <v-card-text>
              ...
              >
                ...
                <v-card-text
                  class="pt-1"
                  v-if="item.name && item.role && item.dateSubmitted"
                >
                  <v-layout column>
                    <v-flex xs12>
                      <div class="hildaLight">
                        <v-icon>account_circle</v-icon> {{ item.name }},
                        {{ item.role }}
                      </div>
                    </v-flex>
                    <div class="ml-4 pl-1">
                      ...
                      <v-flex
                        xs12
                        class="mt-3"
                        v-if="item.remarks && item.remarks.length > 0"
                      >
